...
///
/// Uses a TUN interface on the node (same as the proxy) instead of raw sockets.
///
...
///   - Requires CAP_NET_ADMIN (same as proxy)
///
/// The proxy sends us the list of subnets to route through the tunnel.
/// We create imugi-node0, add those routes, and start forwarding.
...
const TUN_ADDR: &str    = "240.0.0.2";     // proxy is 240.0.0.1
...
impl NodeTun {
    ...
    fn add_route(&self, subnet: &str) -> Result<()> {
        ...
        info!("Route added: {} via {}", subnet, self.name);
        Ok(())
    }
}
...
pub async fn run_forwarding(
    ...
    // Single task owns the TUN device — reads and writes, no locking needed
    loop {
        tokio::select! {
            // TUN → proxy
