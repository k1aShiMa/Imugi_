...
impl TunDevice {
    pub fn new(name: &str, addr: &str, netmask: &str) -> Result<Self> {
        #[cfg(unix)]
        {
            ...
            info!("TUN interface '{}' up at {}", name, addr);
            Ok(TunDevice { name: name.to_owned(), inner: dev })
        }
...
pub fn enable_ip_forwarding() -> Result<()> {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let out = Command::new("sysctl")
            ...
            .output()
            .context("Failed to enable IP forwarding via sysctl")?;
        if !out.status.success() {
            let stderr = String::from_utf8_lossy(&out.stderr);
            bail!("sysctl failed: {}", stderr.trim());
        }
        info!("IP forwarding enabled via sysctl");
    }
