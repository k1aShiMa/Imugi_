...
use crate::{
    protocol::{
        self, AgentHello, AgentMessage, ProxyCommand, DATA_HEADER_LEN, MAGIC,
    },
...
async fn handle_agent(
    ...
    if ver != protocol::VERSION {
        bail!("Version mismatch: got {}, want {}", ver, protocol::VERSION);
    }

    // --- Read AgentHello ---
    let hello: AgentHello = read_json_msg(&mut tls).await.context("Read AgentHello")?;
    info!(
    ...
    // --- Send StartTunnel ---
    let start = ProxyCommand::StartTunnel {
        session_id: session_id.clone(),
    };
    ...
    tokio::spawn(async move {
        while let Some(pkt) = to_agent_rx.recv().await {
            let framed = protocol::encode_packet(&pkt);
    ...
    loop {
        ...
        let len = protocol::decode_len(&header);
        if len == 0 || len > 65535 {
